...
describe('Validating header and navigation elements', () => {
  ...
  it('validate certification navigation', () => {
    cy.get(selectors.data_anchor_certifications).find('p').contains('CERTIFICATIONS').should('exist');
  });

  it('validate interests navigation', () => {
    cy.get(selectors.data_anchor_interests).find('p').contains('INTERESTS AND HOBBIES').should('exist');
  });
